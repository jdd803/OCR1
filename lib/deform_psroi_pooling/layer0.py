...
from lib.deform_psroi_pooling.ps_roi0 import ps_roi, ps_roi1


# cls PSROI pooling
class PsRoiOffset(tf.keras.layers.Layer):
    def __init__(self, pool_size, feat_stride, init_normal_stddev=0.01, **kwargs):
    ...
    def call(self, inputs):
        ...
        pooled_response = ps_roi1(features=features, boxes=rois, offsets=offset,
                                  k=self.pool_size, feat_stride=self.feat_stride)   # (n,depth,h,w)

        return pooled_response


# bbox Psroi-pooling
class PsRoiOffset1(tf.keras.layers.Layer):
    def __init__(self, pool_size, feat_stride, init_normal_stddev=0.01, **kwargs):
        super(PsRoiOffset, self).__init__()
        self.filters = 4*cfg.network.PSROI_BINS*cfg.network.PSROI_BINS
        self.pool_size = pool_size   # control nums of relative positions
