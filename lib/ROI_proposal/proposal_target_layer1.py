...
def proposal_target_layer(rpn_rois, gt_boxes, _num_classes):
    ...
    keep_inds, fg_num = tf.compat.v1.py_func(_proposal_target_layer_py,[rpn_rois, gt_boxes,_num_classes],
                                   [tf.float32, tf.int32,tf.float32, tf.float32,tf.float32, tf.int32, tf.int32])

    rois = tf.reshape(rois, [-1, 5], name='rois')
    # preds = tf.reshape(preds, [-1,5], name='preds')
    # cls_score = tf.reshape(cls_score, [-1, _num_classes + 1], name='cls_score')
