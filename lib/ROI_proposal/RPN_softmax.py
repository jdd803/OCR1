...
def rpn_softmax(rpn_cls_score):
    ...
    two tranpsoses
    '''
    with tf.compat.v1.variable_scope('rpn_softmax'):
        ...
        rpn_cls_prob = tf.transpose(a=rpn_cls_prob, perm=[0, 3, 1, 2])
        rpn_cls_prob = tf.reshape(rpn_cls_prob, [shape[0], shape[3], shape[1], shape[2]])
        rpn_cls_prob = tf.transpose(a=rpn_cls_prob, perm=[0, 2, 3, 1])
