...
def model_part1(images,weight_decay=1e-5,is_training=True,gt_boxes = None):
    ...
    with tf.compat.v1.variable_scope('feature_fusion',values=[endpoints.values]):
        batch_norm_params = {
            'decay':0.997,
            'epsilon':1e-5,
            'scale':True,
        ...
        with slim.arg_scope([slim.conv2d],
            ...
            f_stage4 = upsample_image(fea[1],tf.shape(input=f_stage3)[1],tf.shape(input=f_stage3)[2])
            ...
            f_stage5_2 = upsample_image(f_stage5_1,tf.shape(input=f_stage3)[1],tf.shape(input=f_stage3)[2])
...
def model_part2_1(roi,ps_score_map):
    ...
    cls_result = tf.argmax(cls,axis=0)    #(n,)
    cls_score = tf.argmax(input=cls,axis=-1)
...
def model_part2(imdims,rois,ps_score_map,bbox_shift):
    ...
    proposals = tf.compat.v1.py_func(bbox_transform_inv,[rois[:,-4:],offsets],tf.float32) #(n,4)
    proposals = tf.compat.v1.py_func(clip_boxes,[proposals,imdims],tf.float32)    #(n,4)
    ...
    for i in range(roi_num):
        ...
        rois = tf.concat((rois, proposals), axis=0)
    '''
    roi_num = tf.convert_to_tensor(value=cfg.TRAIN.RPN_POST_NMS_TOP_N*2)
    ...
    def body2(i,rois):
        ...
        j = tf.compat.v1.py_func(_add_to_dict,[i,cls,cls_result,cls_score,mask_result,rois[i],bbox],tf.int32)
        # global result
    ...
    i1,rois = tf.while_loop(cond=cond2, body=body2, loop_vars=(0,rois))
    '''
    for i in range(roi_num):
        bbox = model_part2_2(rois[i], bbox_shift)
        cls, cls_result, cls_score, mask_result = model_part2_1(rois[i], ps_score_map)
...
def model_part3(results):
    ...
    for i in keep:
        keep1 = tf.compat.v1.py_func(nms1,[i,keep,roi,0.5],tf.int32)
        ...
        mask_i,roi_i = tf.compat.v1.py_func(mask_transform,[mask_i,roi_i],[tf.float32,tf.float32])
        ...
        for j in keep1:
            ...
            mask_j, roi_j = tf.compat.v1.py_func(mask_transform, [mask_j, roi_j], [tf.float32, tf.float32])
